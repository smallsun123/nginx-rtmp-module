...
static void 
RTMP_LogHexString(ngx_log_t *log, const uint8_t *data, unsigned long len)
{
#define BP_OFFSET 9
#define BP_GRAPH 60
#define BP_LEN	80
	...
	for ( i = 0 ; i < len ; i++ ) {
		...
		if( !n ) {
			if( i ) //RTMP_Log( level, "%s", line );
				ngx_log_error(NGX_LOG_ERR, log, 0, "%s", line);
...
//		ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
 //                   "RTMP header fmt=%d, csid=%D, time=%uD, exttime=%uD, mlen=%D, len=%D, type=%s(%d), msid=%D\n",
  //                  (int)fmt, csid, h->timestamp, st->dtime, h->mlen, st->len, ngx_rtmp_message_type(h->type), (int)h->type, h->msid);
...
ngx_rtmp_send(ngx_event_t *wev)
{
    ...
    while (s->out_chain) {
	//RTMP_LogHexString(c->log, s->out_bpos, s->out_chain->buf->last - s->out_bpos);
